...
let WizardService = WizardService_1 = class WizardService {
    ...
    async prepareWizard(chatId) {
        ...
        const profile = await this.servicesProvider.profileTelegramService.findByTelegram(telegramChannelId);
        const wizard = profile
            ? new profile_wizard_1.ProfileWizard(profile, this.servicesProvider)
    ...
    wizardLog(wizard, log) {
        const unitIdentifierLog = wizard instanceof profile_wizard_1.ProfileWizard ? ` [${wizard.getProfile().uid}]` : '';
